...
public class DatastoreServer
{
	...
	{
		// Need to implement
		String input=StreamUtil.readLine(inputStream);
		if ("write".equalsIgnoreCase(input)) {
			...
			return command;
		}else if ("Directory".equalsIgnoreCase(input)) {
			ServerCommand command = new DirectoryCommand();
			return command;
		}
		else{
			throw new ServerException("Unknown Request: " + input);
		}
	}
